...
int main(int argc, char *argv[]) {
  static char cvsid[]="$Id: setup_camera.c,v 1.1 2005/08/26 09:03:33 dach Exp $";
  ...
  unsigned char *ptr;
   ...
   if(!strcmp(argv[1],"framerate")) { 
   	if(!strcmp(argv[2],"show")) { 
		switch (old_framerate) {
			...
			case FRAMERATE_120 :{
				printf("FRAMERATE_120\n"); 
				exit(0);
			}
			case FRAMERATE_240 :{
				printf("FRAMERATE_240\n"); 
				exit(0);
			}
		}
	}
   	...
   	if( (!strcmp(argv[2],"FRAMERATE_120")) || (!strcmp(argv[2],"120")) ) { 
		framerate = FRAMERATE_120;
		if (!(framerates & (0x1 <<25))) {
			printf("framerate 120 (FRAMERATE_120) not supported by this camera %X\n",framerates);
			exit(1);
		}
   	}
   	if( (!strcmp(argv[2],"FRAMERATE_240")) || (!strcmp(argv[2],"240")) ) { 
		framerate = FRAMERATE_240;
		if (!(framerates & (0x1 <<24))) {
			printf("framerate 240 (FRAMERATE_240) not supported by this camera %X\n",framerates);
			exit(1);
		}
   	}
	if(!framerate) {
   ...
   if(!strcmp(argv[1],"trigger_delay")) { 
	feature = FEATURE_TRIGGER_DELAY;
   } 
   if(!strcmp(argv[1],"white_shading")) { 
	feature = FEATURE_WHITE_SHADING;
   } 
   if(!strcmp(argv[1],"frame_rate")) { 
	feature = FEATURE_FRAME_RATE;
   } 
   if(!feature){
	fprintf( stderr, "\n" "feature not known\n");
	exit(1);
